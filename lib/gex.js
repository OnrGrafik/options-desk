...
export function calcVolSurface(options,spot){
  ...
  for(const exp of Object.values(em).sort((a,b)=>a.ts-b.ts)){
    ...
    for(const o of opts){if(!o.T||!o.iv)continue;const{d1}=d1d2(spot,o.strike,o.T,o.iv,0,0);const delta=o.type==="call"?normCDF(d1):normCDF(d1)-1;if(o.type==="call"){const dist=Math.abs(delta-0.25);if(dist<minCd){minCd=dist;bestCall25={...o,calcDelta:delta};}}else{const dist=Math.abs(Math.abs(delta)-0.25);if(dist<minPd){minPd=dist;bestPut25={...o,calcDelta:delta};}}}
    if(bestCall25&&bestPut25&&minCd<0.15&&minPd<0.15){const rr=(bestPut25.iv-bestCall25.iv)*100;if(Math.abs(rr)<20)riskReversals.push({days,rr,putIV:bestPut25.iv*100,callIV:bestCall25.iv*100,atmIV:atmIV?atmIV*100:null});}
  }
