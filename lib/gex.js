...
export function calcZeroGamma(allOptions, spot) {
  if(!allOptions?.length) return null;
  const now=Date.now();
  const lo=spot*0.70, hi=spot*1.30, steps=200, step=(hi-lo)/steps;
  const profile=[];
  for(let i=0;i<=steps;i++) {
    const S=lo+i*step; let total=0;
    for(const o of allOptions) {
      const T=Math.max((o.expiryTs-now)/(365.25*24*3600*1000),0.0001);
      total+=bsGamma(S,o.strike,T,o.iv)*o.oi*S*S*0.01*(o.type==="call"?1:-1);
    }
    profile.push({price:S,gex:total});
  }
  const crossings=[];
  for(let i=1;i<profile.length;i++) {
    const p=profile[i-1],c=profile[i];
    if((p.gex>0)!==(c.gex>0)) {
      const ratio=Math.abs(p.gex)/(Math.abs(p.gex)+Math.abs(c.gex));
      crossings.push({price:Math.round(p.price+ratio*(c.price-p.price)),fromPositive:p.gex>0});
    }
  }
