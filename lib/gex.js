...
export async function fetchAllOptions(instruments, spot, onProgress) {
  ...
  for(let i=0;i<filtered.length;i+=batchSize) {
    ...
    for(let j=0;j<batch.length;j++) {
      ...
      // ── Deribit'in kendi greek'lerini kullan (forward fiyatlı, skew düzeltmeli) ──
      let gamma=0, delta=0, vega=0, vannaBs=0, charmBs=0;
      if(tk.greeks&&typeof tk.greeks.gamma==="number") {
        gamma = tk.greeks.gamma;   // Deribit: ∂delta/∂S — per $1 move
        delta = tk.greeks.delta||0;
        vega  = tk.greeks.vega||0; // Deribit: ∂V/∂σ (option value change per 1% IV)
      } else {
      ...
      // Formül (SpotGamma/Barchart standardı):
      // GEX = Gamma × OI × S² × 0.01 × sign
      // Deribit gamma birimi: ∂delta/∂S (per $1), bu yüzden S² × 0.01 → per 1% move
      // Piyasa yapıcı varsayımı: müşteri call alır → MM short call → MM long gamma (+)
      //                          müşteri put alır → MM short put → MM short gamma (-)
      const gex = gamma * oi * spot * spot * 0.01 * (type==="call" ? 1 : -1);

      // ── Vanna Exposure (VEX) ──────────────────────────────
      // Formül (VannaCharm/Option Screener standardı):
      // VEX = OI × Vanna × S × σ
      // Vanna = ∂delta/∂σ = ∂gamma/∂σ × ... = vega × d1/(S×σ×√T) [BS formulü]
      // Deribit vega birimi: ∂V/∂(IV%), yani 1% IV değişimi başına $ değişim
      // Vanna hesabı: BS ile d1 üzerinden
      let vannaEx = 0;
      if(T>0&&iv>0) {
        const sqrtT=Math.sqrt(T);
        const d1=(Math.log(spot/strike)+(0.5*iv*iv)*T)/(iv*sqrtT);
        const bsVanna=normPDF(d1)*d1/(spot*iv); // ∂delta/∂σ (normalized)
        // VEX: OI × Vanna × S × σ (IV dahil) — dollar terms per 1% IV change
        vannaEx = bsVanna * oi * spot * iv * (type==="call" ? 1 : -1);
      }

      // ── Charm Exposure (CEX) ──────────────────────────────
      // Formül: Charm = ∂delta/∂τ (τ = time to expiry)
      // BS Charm (call): -normPDF(d1) × [r/(σ√T) - d2/(2T)]
      // Simplified for r≈0: Charm ≈ -normPDF(d1) × d2 / (2T)
      // CEX = OI × |Charm| × S (dollars per 1-day time decay)
      let charmEx = 0;
      if(T>0&&iv>0) {
        const sqrtT=Math.sqrt(T);
        const d1=(Math.log(spot/strike)+(0.5*iv*iv)*T)/(iv*sqrtT);
        const d2=d1-iv*sqrtT;
        // Charm = -normPDF(d1) × [d2 / (2T)] (r=0 varsayımı)
        const charm=-normPDF(d1)*(d2/(2*T));
        // CEX: OI × Charm × S (per 1 day)
        charmEx = charm * oi * spot * (type==="call" ? 1 : -1);
      }
...
// ─── VOLATİLİTE YÜZEYİ HESAPLAMALARI ─────────────────────
//
// ATM Term Structure:
// Her vade için ATM IV = spot'a en yakın iki strike'ın IV'sinin
// log-moneyness ağırlıklı interpolasyonu.
// ATM = |log(K/S)| en küçük olan strike değil,
//        BSM forward = K kondisyonuyla interpolasyon.
//
// 25-Delta Risk Reversal:
// RR(25Δ) = IV(25Δ put) - IV(25Δ call)  [vol noktaları cinsinden]
// Pozitif RR → put daha pahalı → downside bias
// Negatif RR → call daha pahalı → upside bias
// 25Δ strike'ı bulmak için: mevcut opsiyon zincirinde |delta - 0.25| minimumunu bul
// ─────────────────────────────────────────────────────────
...
export function calcVolSurface(options, spot) {
  ...
  for (const o of options) {
    ...
    if (!expiryMap[o.expiryTs]) {
      expiryMap[o.expiryTs] = { ts: o.expiryTs, days: o.daysToExp, T: o.T, opts: [] };
    }
    expiryMap[o.expiryTs].opts.push(o);
  }
  ...
  for (const exp of expiries) {
    ...
    // Her opsiyon için log(K/S) hesapla
    // En küçük |log(K/S)| iki strike'ı bul (birinden küçük, birinden büyük)
    ...
    // ATM strike: call OI en yüksek olan spot'a yakın strike
    // veya log-moneyness = 0'a en yakın
    let atmIV = null;
    if (calls.length) {
      // Spot'un hemen üstündeki call ve hemen altındaki call
      const callAbove = calls.find(o => o.strike >= spot);
      const callBelow = [...calls].reverse().find(o => o.strike < spot);

      if (callAbove && callBelow) {
        // Log-moneyness interpolasyonu:
        // ATM IV ≈ IV(K1) × w + IV(K2) × (1-w)
        // w = log(K2/S) / log(K2/K1)
        const lm1 = Math.log(callBelow.strike / spot); // negatif
        const lm2 = Math.log(callAbove.strike / spot);  // pozitif
        if (Math.abs(lm2 - lm1) > 0) {
          const w = Math.abs(lm1) / (Math.abs(lm1) + Math.abs(lm2));
          atmIV = callBelow.iv * (1 - w) + callAbove.iv * w;
        } else {
          atmIV = callAbove.iv;
        }
      } else if (callAbove) {
      ...
      // Put'larla çapraz kontrol (put-call parity'den ATM kontrolü)
      if (atmIV === null && puts.length) {
        const putAtm = puts.reduce((b, o) => Math.abs(o.strike - spot) < Math.abs(b.strike - spot) ? o : b, puts[0]);
        atmIV = putAtm.iv;
      }
    }

    if (atmIV !== null) {
      termStructure.push({ days, T, iv: atmIV * 100 }); // % cinsinden
    }

    // ── 25-Delta Risk Reversal ────────────────────────────
    // 25Δ call ve 25Δ put'u bul
    // Delta kriterini Deribit'in kendi delta'sından hesapla
    // 25Δ call: delta en yakın +0.25 olan call
    // 25Δ put:  delta en yakın -0.25 olan put (|delta| ≈ 0.25)

    let bestCall25 = null, bestPut25 = null;
    let minCallDist = Infinity, minPutDist = Infinity;

    for (const o of opts) {
      // Deribit delta kullan (varsa), yoksa BS hesapla
      let delta = o.delta;
      if (!delta || isNaN(delta)) {
        if (o.T > 0 && o.iv > 0) {
          const sqrtT = Math.sqrt(o.T);
          const d1 = (Math.log(spot / o.strike) + 0.5 * o.iv * o.iv * o.T) / (o.iv * sqrtT);
          delta = o.type === "call" ? normCDF(d1) : normCDF(d1) - 1;
        } else {
      ...
      if (o.type === "call") {
        const d = Math.abs(delta - 0.25);
        if (d < minCallDist) { minCallDist = d; bestCall25 = o; }
      } else {
        // Put delta negatif: |delta| ≈ 0.25
        const d = Math.abs(Math.abs(delta) - 0.25);
        if (d < minPutDist) { minPutDist = d; bestPut25 = o; }
      }
    }

    if (bestCall25 && bestPut25) {
      // RR = IV(25Δ put) - IV(25Δ call) [vol noktası]
      const rr = (bestPut25.iv - bestCall25.iv) * 100;
      // Sadece makul değerleri al (±20 vol noktası)
      if (Math.abs(rr) < 20) {
        riskReversals.push({
          days,
          rr,
