...
export function calcMaxPain(allOptions){
  ...
  for(const K of strikes){let pain=0;for(const s of strikes){if(s.strike>K.strike)pain+=(s.strike-K.strike)*s.callOI;if(K.strike>s.strike)pain+=(K.strike-s.strike)*s.putOI;}if(pain<minPain){minPain=pain;maxPain=K.strike;}}
  return maxPain;
}
