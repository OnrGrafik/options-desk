...
function useCountUp(target, ms=700) {
  ...
  useEffect(()=>{ if(target===prev.current) return; const s=prev.current,t0=performance.now(); let f; const tick=n=>{const t=Math.min((n-t0)/ms,1),e=1-Math.pow(1-t,3);setV(s+(target-s)*e);if(t<1)f=requestAnimationFrame(tick);else prev.current=target;}; f=requestAnimationFrame(tick); return()=>cancelAnimationFrame(f); },[target,ms]);
  return v;
}

// ─── Per-asset data hook ───────────────────────────────────
function useAssetData(sym, vadeFiltresi) {
  const [raw, setRaw] = useState({ spot:0, allOptions:[], ticker24h:{open:0,high:0,low:0,change:0,volume:0}, funding:0, basis:0, dvol:50, loading:true, error:null, progress:"", lastUpdate:null, stats:{rows:0,totalInst:0,expiries:0} });
  const cache = useRef({});
  ...
  const load = useCallback(async (sessiz=false) => {
    ...
    try {
      ...
      setRaw(s=>({...s,spot:spot||0,ticker24h:ticker24h||s.ticker24h,funding:funding||0,basis:basis||0,progress:`${sym} opsiyon zinciri çekiliyor...`}));
      ...
      const { options, stats } = await fetchAllOptions(instruments, spot||0, sym, (pct,rows,exps)=>{
        setRaw(s=>({...s,progress:`${sym} analiz: %${pct} · ${rows} opt · ${exps} vade`}));
      });

      const atmOpt = options.filter(o=>o.type==="call").sort((a,b)=>Math.abs(a.strike-spot)-Math.abs(b.strike-spot))[0];
      setRaw(s=>({...s,spot:spot||0,allOptions:options,ticker24h:ticker24h||s.ticker24h,funding:funding||0,basis:basis||0,stats,dvol:atmOpt?atmOpt.iv*100:50,loading:false,error:null,lastUpdate:new Date(),progress:""}));
    } catch(e) {
      setRaw(s=>({...s,loading:false,error:e.message}));
    }
  },[sym]);

  useEffect(()=>{ load(); const iv=setInterval(()=>load(true),5*60*1000); return()=>clearInterval(iv); },[load]);
...
function hullYorum(data, sym) {
  ...
  const band=cw-pw, bandPct=((band/spot)*100).toFixed(1);
  ...
  const putBias=frontRR?.rr>0, rrVal=frontRR?.rr?.toFixed(1);
  const rejiim=posGamma?`Hull (Bölüm 19.6): Dealer net long gamma — piyasa yapıcılar fiyat yükseldikçe ${sym} satıyor, düştükçe alıyor. Bu karşı-trend (mean-reversion) akışı volatiliteyi baskılar. Spot ${fmt(cw)} Call Wall'una yaklaştıkça delta hedging baskısı artar.`:`Hull (Bölüm 19.6): Dealer net short gamma — piyasa yapıcılar trendle aynı yönde işlem yapıyor. Bu momentum etkisi volatiliteyi amplify eder. Zero Gamma ${fmt(zeroGamma)} seviyesinin üstüne çıkış kritik.`;
  const mpYorum=distToMP?`Hull (Bölüm 19.5): Spot, Max Pain ${fmt(maxPain)} seviyesinin %${Math.abs(parseFloat(distToMP)).toFixed(1)} ${parseFloat(distToMP)>0?"üstünde":"altında"}. Vadeye yaklaştıkça Charm etkisiyle (∂Δ/∂t) pin baskısı ${fmt(maxPain)}'e doğru güçlenir.`:"";
  ...
  const volYorum=termSlope&&shortIV&&longIV?`Hull (Bölüm 20.5): IV term structure ${termSlope==="contango"?"normal eğimli (contango)":"ters (backwardation)"}; kısa vade ${shortIV}%, uzun vade ${longIV}%. ${rrVal?(putBias?`25Δ Risk Reversal +${rrVal} vol (put bias) — piyasa aşağı hareket için prim ödüyor.`:`25Δ Risk Reversal ${rrVal} vol (call bias) — piyasa yukarı hareket için prim ödüyor.`):""}`:""  ;
  ...
  return {rejiim,mpYorum,vannaYorum,volYorum,emYorum,posGamma,netGexStr,cw,pw,band,distToCW,distToPW};
}
...
function Sidebar({ watchlist, activeSym, setActiveSym, vade, setVade, data }) {
  ...
  return (
    <aside className="sidebar">
      ...
      {/* Watchlist — her satır tıklanabilir */}
      <div className="sb-section">
        <div className="sb-label">Varlıklar</div>
        {watchlist.map(w => {
          const asset = ASSETS[w.sym];
          const isActive = w.sym === activeSym;
          return (
            <div
              key={w.sym}
              className={`sb-item ${isActive?"active":""}`}
              style={{ cursor:"pointer" }}
              onClick={() => setActiveSym(w.sym)}
            >
              <span className="sb-item-key tabular" style={{ display:"flex", alignItems:"center", gap: 5 }}>
                <span style={{ width:6, height:6, borderRadius:"50%", background: asset?.color||"var(--accent)", display:"inline-block", flexShrink:0 }} />
                {w.sym}/USD
              </span>
              <span className="sb-item-val">
                <span style={{ color:"var(--text)" }}>
                  {w.price ? w.price.toLocaleString("tr-TR",{maximumFractionDigits:w.price<10?4:w.price<100?2:0}) : "—"}
                </span>
  ...
  return (
    <div className="ladder" style={{position:"relative"}}>
      ...
      {hovered&&(()=>{
        ...
        return (
          <div style={{position:"absolute",top:0,right:0,background:"var(--surface)",border:"1px solid var(--hairline-strong)",borderRadius:4,fontFamily:"var(--sans)",fontSize:11,pointerEvents:"none",zIndex:200,minWidth:215,overflow:"hidden",boxShadow:"0 4px 24px rgba(0,0,0,0.5)"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 12px",borderBottom:"1px solid var(--hairline)",background:"var(--surface-2)"}}>
              <span style={{fontSize:14,fontWeight:700,color:"var(--text)"}}>${fmt(hovered.strike)}</span>
      ...
      {vis.map((s,i)=>{
        ...
        const callPct=s.callGex/maxCall*100, putPct=Math.abs(s.putGex)/maxPut*100;
          ...
          <Fragment key={s.strike}>
            {i===spotIdx&&(
              <div className="ladder-row spot">
                ...
                <div className="strike-cell tabular" style={{color:"var(--accent)",fontWeight:700}}>{fmt(spot)}</div>
...
function QuantumWalls({data, sym}) {
  ...
  return (
    <div style={{display:"grid",gridTemplateColumns:"1fr 290px",gap:32}}>
      <div>
        ...
        <div style={{position:"relative"}} onMouseLeave={()=>{setIpucu(null);setHover(null);}}>
          <svg ref={svgRef} viewBox={`0 0 ${W} ${H}`} style={{width:"100%",height:"auto",display:"block"}} onMouseMove={handleFare}>
            ...
            {vis.map(s=>{const y=yS(s.strike);return(<g key={`iz-${s.strike}`}><line x1={pad.left} x2={W-pad.right} y1={y} y2={y} stroke="var(--hairline-soft)" strokeWidth="0.3"/><text x={pad.left-6} y={y+3} textAnchor="end" fontFamily="var(--sans)" fontSize="10" fontWeight="700" fill="var(--text-mute)">{s.strike>=1000?(s.strike/1000).toFixed(0)+"K":s.strike.toFixed(2)}</text></g>);})}
            {vis.map(s=>{
            ...
            {[0,0.25,0.5,0.75,1].map(p=>{const x=pad.left+p*cW*0.92,v=p*maxBar;return(<g key={`xek-${p}`}><line x1={x} x2={x} y1={H-pad.bottom} y2={H-pad.bottom+4} stroke="var(--hairline)"/><text x={x} y={H-pad.bottom+16} textAnchor="middle" fontFamily="var(--sans)" fontSize="9" fontWeight="700" fill="var(--text-mute)">${fmtB(v)}</text></g>);})}
          ...
          {ipucu&&(()=>{
            const wl=ipucu.wallType==="callWall"?{txt:"CALL WALL",renk:"var(--pos)"}:ipucu.wallType==="putWall"?{txt:"PUT WALL",renk:"var(--neg)"}:ipucu.wallType==="magnet"?{txt:"MAGNET",renk:"var(--neutral)"}:{txt:"NEUTRAL",renk:"var(--text-dim)"};
            return(<div style={{position:"absolute",top:8,right:8,background:"var(--surface)",border:"1px solid var(--hairline-strong)",borderRadius:4,fontFamily:"var(--sans)",fontSize:11,pointerEvents:"none",zIndex:100,minWidth:200,overflow:"hidden"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 12px",borderBottom:"1px solid var(--hairline)",background:"var(--surface-2)"}}>
              ...
              {[["Net GEX",(ipucu.netGex>=0?"+":"−")+fmtM(Math.abs(ipucu.netGex)),ipucu.netGex>=0?"var(--pos)":"var(--neg)"],["Call GEX","+"+fmtM(ipucu.callGex),"var(--pos)"],["Put GEX","−"+fmtM(Math.abs(ipucu.putGex)),"var(--neg)"],["OI density",ipucu.oiPct+"%","var(--text)"],["γ density",ipucu.gexPct+"%","var(--text)"]].map(([e,d,r])=>(
                <div key={e} style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",padding:"5px 12px",borderBottom:"1px solid var(--hairline-soft)"}}>
                  <span style={{color:"var(--text-dim)",fontSize:10,fontWeight:600}}>{e}</span>
                  <span style={{color:r,fontWeight:700,fontSize:11}}>{d}</span>
                </div>
              ))}
            </div>);
          })()}
      ...
      <div>
        <p style={{fontFamily:"var(--serif)",fontSize:15,lineHeight:1.45,color:"var(--text-2)",marginBottom:20}}>
          İki <em style={{fontStyle:"italic",color:"var(--accent)"}}>$5K-bant</em> spot'u çerçeveler: yukarıda Call Wall kümesi, aşağıda Put Wall yığını.
        </p>
        ...
        {topWalls.map((w,i)=>{
          ...
          return(<div key={w.strike} style={{display:"grid",gridTemplateColumns:"22px 1fr auto",gap:"6px 10px",alignItems:"baseline",padding:"9px 0",borderBottom:"1px solid var(--hairline-soft)"}}>
            <span style={{fontSize:9,color:"var(--text-mute)",fontStyle:"italic",fontFamily:"var(--sans)"}}>{String(i+1).padStart(2,"0")}</span>
            <div><div style={{fontSize:13,color:renk,fontWeight:700,fontFamily:"var(--sans)"}}>${fmt(w.strike)}</div><div style={{fontSize:9,color:renk,marginTop:2,fontFamily:"var(--sans)",fontWeight:600}}>{alim?"Call":satim?"Put":"Magnet"} · OI {w.oiPct}%</div></div>
            <div style={{textAlign:"right"}}><div style={{fontSize:12,color:pct>=0?"var(--pos)":"var(--neg)",fontFamily:"var(--sans)",fontWeight:700}}>{pct>=0?"+":""}{pct.toFixed(1)}%</div><div style={{fontSize:10,color:"var(--text-mute)",marginTop:2,fontFamily:"var(--sans)"}}>{pct>=0?"+":"-"}${fmtB(Math.abs(w.netGex))}</div></div>
          </div>);
        })}
      </div>
    </div>
...
function TermStructure({data}) {
  ...
  const minIV=Math.floor(Math.min(...pts.map(p=>p.iv))/5)*5-5,maxIV=Math.ceil(Math.max(...pts.map(p=>p.iv))/5)*5+5;
  ...
  return(
    <svg viewBox={`0 0 ${W} ${H}`} className="term-svg">
      ...
      {[7,30,90,180,240].filter(g=>g<=maxDays).map(g=><g key={g}><line x1={xS(g)} x2={xS(g)} y1={H-pad.bottom} y2={H-pad.bottom+4} stroke="var(--hairline-strong)"/><text x={xS(g)} y={H-pad.bottom+16} textAnchor="middle" fontFamily="var(--sans)" fontSize="9" fontWeight="700" fill="var(--text-mute)">{g}g</text></g>)}
...
function RiskReversal({data}) {
  ...
  const maxG=Math.max(...rows.map(e=>e.days)),maxRR=Math.max(...rows.map(e=>e.rr))+1.5,minRR=Math.min(0,Math.min(...rows.map(e=>e.rr))-0.5);
...
// ─── ASSET PAGE (BTC ile aynı görsel, farklı varlık) ───────
function AssetPage({ sym, vade, watchlist }) {
  const data = useAssetData(sym, vade);
  ...
  if (data.loading) return (
    <div style={{minHeight:"calc(100vh - 60px)",display:"grid",placeItems:"center",color:"var(--text-mute)",fontFamily:"var(--sans)",fontSize:11,letterSpacing:"0.10em"}}>
      <div style={{textAlign:"center"}}>
        <div style={{width:32,height:32,border:"1.5px solid var(--hairline-strong)",borderTopColor:asset.color||"var(--accent)",borderRadius:"50%",animation:"spin 0.9s linear infinite",margin:"0 auto 14px"}}/>
        <div style={{color:asset.color||"var(--accent)",fontWeight:700,marginBottom:8}}>{sym}</div>
  ...
  if (data.error) return (
    <div style={{padding:"40px 36px",color:"var(--neg)",fontFamily:"var(--sans)"}}>
      <div style={{marginBottom:12}}>❌ {sym}: {data.error}</div>
    ...
    <>
      ...
      <div style={{padding:"12px 36px",borderBottom:"1px solid var(--hairline)",display:"flex",justifyContent:"space-between",alignItems:"center",background:"var(--surface)"}}>
        <div style={{display:"flex",gap:16,alignItems:"center"}}>
          <span style={{width:10,height:10,borderRadius:"50%",background:asset.color,display:"inline-block"}}/>
          <span style={{fontFamily:"var(--sans)",fontSize:18,fontWeight:700,color:"var(--text)"}}>{asset.label} ({sym})</span>
          <span style={{fontFamily:"var(--sans)",fontSize:20,fontWeight:700,color:"var(--text)",fontVariantNumeric:"tabular-nums"}}>${data.spot.toLocaleString("tr-TR",{maximumFractionDigits:data.spot<10?4:data.spot<100?2:0})}</span>
          <span style={{fontFamily:"var(--sans)",fontSize:13,fontWeight:700,color:data.ticker24h.change>=0?"var(--pos)":"var(--neg)"}}>
            {data.ticker24h.change>=0?"+":""}{(data.ticker24h.change||0).toFixed(2)}%
          </span>
          <span style={{fontFamily:"var(--sans)",fontSize:11,fontWeight:500,color:pozitif?"var(--pos)":"var(--neg)"}}>
            ● {pozitif?"POZİTİF":"NEGATİF"} GAMMA
          </span>
        </div>
        <div style={{display:"flex",gap:12,alignItems:"center"}}>
          <span style={{fontFamily:"var(--sans)",fontSize:10,fontWeight:600,color:"var(--text-mute)",textTransform:"uppercase",letterSpacing:"0.06em"}}>
            Güncellendi <b style={{color:"var(--text)"}}>{saatStr}</b> UTC
          </span>
          <button onClick={data.yenile} style={{background:"none",border:"1px solid var(--hairline-strong)",color:"var(--text-2)",padding:"3px 10px",fontFamily:"var(--sans)",fontSize:11,fontWeight:600,cursor:"pointer",transition:"all 0.12s"}} onMouseOver={e=>e.target.style.background="var(--surface-2)"} onMouseOut={e=>e.target.style.background="none"}>↻ Yenile</button>
          {asset.linear&&<span style={{fontFamily:"var(--sans)",fontSize:9,fontWeight:600,color:"var(--text-mute)",letterSpacing:"0.08em",textTransform:"uppercase",padding:"2px 6px",border:"1px solid var(--hairline)",background:"var(--surface-2)"}}>USDC-Linear</span>}
        </div>
      </div>
      ...
      <section className="section">
        ...
        <div className="ladder-wrap">
          <StrikeLadder data={data} sym={sym}/>
          <div style={{display:"flex",flexDirection:"column",gap:28}}>
            <KeyLevels data={data}/>
            <HullCommentary data={data} sym={sym}/>
          </div>
        </div>
      </section>
      ...
      <section className="section">
        ...
        <QuantumWalls data={data} sym={sym}/>
      </section>
        ...
        <div className="two-up">
          <div>
            <p className="pull" style={{marginBottom:24}}>
              Put Wall ile Call Wall arasındaki <em>${fmt(cw-pw)}</em> bant gerçekleşen volatiliteyi sınırlar — tepeden dibe <em>{((cw-pw)/sp*100).toFixed(1)}%</em>.
            </p>
            <p style={{fontFamily:"var(--sans)",fontSize:12,fontWeight:400,color:"var(--text-2)",lineHeight:1.7,margin:0}}>
              Dealer'lar bu haftaya {pozitif?"net long":"net short"} {fmtB(data.totals.gamma)}$ gamma ile giriyor, <b style={{color:"var(--text)"}}>{fmt(data.levels.callWall)}</b> Call Wall'unda yoğunlaşmış. Bu yapısal bir <b style={{color:"var(--text)"}}>mean-reversion</b> bias yaratır.
            </p>
          </div>
          <div>
      ...
      <footer className="footer">
        <div>
          <div style={{marginBottom:4,fontFamily:"var(--sans)",fontSize:11,fontWeight:500}}>Options Desk · {asset.label} ({sym}) · {asset.linear?"USDC-Linear":"Inverse"}</div>
...
export default function Home() {
  ...
  const [watchlist, setWatchlist] = useState(Object.values(ASSETS).map(a=>({sym:a.sym,label:a.label,price:0,chg:0})));

  // Watchlist'i periyodik güncelle
  useEffect(()=>{
  ...
  if (data.loading) return (
    <>
      ...
      <div className="app">
        <Sidebar watchlist={watchlist} activeSym={activeSym} setActiveSym={setActiveSym} vade={vade} setVade={setVade} data={{...data,ticker24h:data.ticker24h||{open:0,high:0,low:0,change:0,volume:0}}}/>
        <div style={{display:"grid",placeItems:"center",minHeight:"100vh",color:"var(--text-mute)",fontFamily:"var(--sans)",fontSize:11,letterSpacing:"0.10em",fontWeight:600}}>
          <div style={{textAlign:"center"}}>
            <div style={{width:36,height:36,border:"1.5px solid var(--hairline-strong)",borderTopColor:asset.color||"var(--accent)",borderRadius:"50%",animation:"spin 0.9s linear infinite",margin:"0 auto 16px"}}/>
            <div style={{color:asset.color,fontWeight:700,fontSize:13,marginBottom:8}}>{asset.label}</div>
  ...
  return (
    <>
      <Head><title>Options Desk · {activeSym}</title></Head>
      <div className="app">
        <Sidebar watchlist={watchlist} activeSym={activeSym} setActiveSym={sym=>{ setActiveSym(sym); setVade("all"); }} vade={vade} setVade={setVade} data={data}/>
        ...
        <main className="main">
          ...
          {/* Main asset page */}
          <AssetPage sym={activeSym} vade={vade} watchlist={watchlist}/>
        </main>
      </div>
    </>
  );
